...
class ElectricDataset(Dataset):
    def __init__(self, df, input_len=90, pred_len=365, scaler=None):
...
class LSTMPredictor(nn.Module):
    def __init__(self, input_size, hidden_size=512, num_layers=2, output_len=365, dropout=0.2):
        super().__init__()
...
def plot_predictions(trues, preds, run_idx, indices=[0]):
    for i in indices:
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig(f"pic/run{run_idx+1}_sample{i+1}_prediction.png", dpi=300)
        plt.close()
...
def plot_loss_curve(losses, run_idx):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"pic/run{run_idx+1}_loss_curve.png", dpi=300)
    plt.close()
...
def main(train_path, test_path, input_len=90, pred_len=365, epochs=300, batch_size=32, loss_type="combined"):
